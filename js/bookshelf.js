...
Bookshelf.prototype.template_titleBox = function() {
  ...
  titleBox +=
    '<h2 class="srchTtl ico-bookshelf"><span class="text_ttl js_seriesName">ＭＹ本棚</span></h2>';
  titleBox += "</div>";
...
Bookshelf.prototype.template_listNav_inner1 = function() {
  ...
  listNav_inner1 +=
    '<input type="text" id="filter_bookshelf" name="filter_bookshelf" placeholder="本棚内を検索" value="" class="searchWord_bookshelf" autocomplete="off"><label for="searchWord_bookshelf">検索</label>';
  listNav_inner1 +=
    '<div class="js_filtedItemBox filterBox_bookshelf"><ul class="filter_bookshelf"></ul></div>';
  listNav_inner1 += "</div>";
  ...
  listNav_inner1 +=
    '<div class="js_radioCookie js_display_style styleBtnBox_book">';
  ...
  listNav_inner1 +=
    '<div class="js_radioCookie js_display_num listNumBox_book">';
  listNav_inner1 +=
    '<span checked">' + thisBookshelf.displyItemCount[0] + "</span>";
...
Bookshelf.prototype.template_listNav_inner2 = function() {
  ...
  if (thisBookshelf.pageType == "top") {
    listNav_inner2 +=
      '<div class="js_radioCookie js_topItem_order orderSelect_book">';
    ...
    listNav_inner2 +=
      '<div class="js_radioCookie js_detailItem_order orderSelect_book">';
...
Bookshelf.prototype.template_itemCount = function(startNum, endNum, totalNum) {
  $(".js_startNum").text(startNum + 1);
  $(".js_endNum").text(endNum);
  $(".js_totalNum").text(totalNum);
};
Bookshelf.prototype.template_pager = function(js_now_page, totalPageNum) {
  var pagerElements = "";
  pagerElements +=
    '<li class="first"><a href=""><img src="/test/pre_bookrenewal/img/first.png" alt="pager to top"></a></li>';
  pagerElements +=
    '<li class="prev"><a href=""><img src="/test/pre_bookrenewal/img/prev.png" alt="pager to one before"></a></li>';
  ...
  pagerElements += "</li>";
  pagerElements +=
    '<li class="next"><a href=""><img src="/test/pre_bookrenewal/img/next.png" alt="pager to next"></a></li>';
  pagerElements +=
    '<li class="end"><a href=""><img src="/test/pre_bookrenewal/img/end.png" alt="pager to end"></a></li>';
  var oldPagerObj = $(".js_now_page").children("li");
  if (oldPagerObj.length) {
    $(".js_now_page").empty();
  }
  $(".js_now_page").append(pagerElements);
};
Bookshelf.prototype.template_cartModal = function(seriesData, status, index) {
  ...
  if (status == "unread") {
    ...
    cartModal +=
      '<p class="js_ttl_modalCart_bookshelf">' + seriesData.item_name + "</p>";
    if (seriesData.now_price) {
      cartModal += '<dl class="priceBox_modalCart_bookshelf">';
      cartModal +=
        '<dt class="ttl_price_modalCart_bookshelf strikethrough">価格：</dt>';
      cartModal +=
        '<dd class="js_price_modalCart_bookshelf strikethrough">' +
        seriesData.unit_price +
        "</dd>";
      cartModal += '<dd class="unit_price_modalCart_bookshelf">円 (税込) </dd>';
      cartModal += "</dl>";
      cartModal += '<dl class="priceBox_modalCart_bookshelf">';
      cartModal +=
        '<dt class="ttl_price_modalCart_bookshelf">キャンペーン価格：</dt>';
      cartModal +=
        '<dd class="js_price_modalCart_bookshelf">' +
        seriesData.now_price +
        "</dd>";
      cartModal += '<dd class="unit_price_modalCart_bookshelf">円 (税込) </dd>';
      cartModal += "</dl>";
    } else {
    ...
    cartModal += '<dl class="priceBox_modalCart_bookshelf">';
    cartModal +=
      '<dt class="ttl_point_modalCart_bookshelf">基礎ポイント：</dt>';
    cartModal +=
      '<dd class="js_point_modalCart_bookshelf">' + seriesData.point + "</dd>";
    cartModal += '<dd class="unit_point_modalCart_bookshelf">pt</dd>';
    cartModal += "</dl>";

    cartModal += '<dl class="priceBox_modalCart_bookshelf">';
    cartModal +=
      '<dt class="ttl_point_modalCart_bookshelf">ポイント増量：</dt>';
    cartModal +=
      '<dd class="js_point_modalCart_bookshelf">' +
      seriesData.point +
      "pt </dd>";
    cartModal +=
      '<dd class="unit_point_modalCart_bookshelf"> (' +
      seriesData.point +
      "倍)</dd>";
    cartModal += "</dl>";
    ...
    cartModal += "</div>";
    var href =
      "javascript:with(document.forms['cart']){method='post';action='https://testbook.hikaritv.net/shopping/app/catalog/detail_book/addcart/book-store/'" +
      seriesData.item +
      "';if(nttinstallmentFlg('false', '')){javascript:blockDoubleSubmit(document.forms['cart'])}else{dummy();}}";
    cartModal += '<div class="btnBox_cart_modalCart_bookshelf">';
    cartModal +=
      '<a class="js_btn_addcart_modalCart_bookshelf" href="' +
      href +
      '">カートに入れる</a> ';
    cartModal += "</div>";
    cartModal += '<div class="linkBox_modalCart_bookshelf">';
    cartModal +=
      '<a href="https://testbook.hikaritv.net/book/content/book-store/' +
      seriesData.userID +
      '">この商品の詳細を確認する</a>';
    cartModal += "</div>";
    ...
    cartModal += '<div class="modalCart_bookshelf">';
    cartModal +=
      '<p class="ttl_modalCart_bookshelf">この巻は予約中です。<br>YYYY年MM月NN日に配信開始になります。</p>';
    cartModal += "</div>";
    cartModal += '<div class="linkBox_modalCart_bookshelf">';
    cartModal +=
      '<a href="https://testbook.hikaritv.net/book/content/book-store/' +
  ...
  $(document).ready(function() {
    $(".item" + index).append(cartModal);
  });
};
Bookshelf.prototype.template_confModal = function() {
  var confModal = "";
  confModal +=
    '<div class="js_confModal_bookshelf wrapper_confModal_bookshelf">';
  confModal += '<span class="js_confModal_close">✕</span>';
  confModal += '<div class="confModal_bookshelf">';
  confModal +=
    '<h2 class="ttl_confModal_bookshelf line-book">本棚の設定画面</h2 >';
  confModal +=
    '<p class="lead_confModal_bookshelf">本棚を設定を登録することができます。使用始めのとき、熱により多少樹脂の臭いがありますが、120℃の消毒保管庫で、2～3時間、室内の換気をよくして加熱してください。臭いはなくなります。臭いは問題ありません。</p>';
  confModal += '<ol class="list_attension_confModal_bookshelf">';
  confModal += "<li>比重1.6～2.2　耐熱温度180℃</li>";
  confModal +=
    "<li>不飽和ポリエステルとガラス繊維の複合樹脂で、耐衝撃性に優れています。</li>";
  confModal +=
    "<li>成形品の形状によりガラス繊維の長さが異なります。トレイ（長）食器（短）</li>";
  confModal +=
    "<li>使用始めのとき、熱により多少樹脂の臭いがありますが、120℃の消毒保管庫で、2～3時間、室内の換気をよくして加熱してください。臭いはなくなります。臭いは問題ありません。</li>";
  confModal += "<li>比重0.9　耐熱温度120℃</li>";
  confModal += "<li>プラスチック食器の中では最も軽く、耐薬品性が高い。</li>";
  confModal += "<li>材質は軟らかく、キズがつきやすい。</li>";
  confModal +=
    "<li>汚れをそのままにしておくと、樹脂の内部に汚れが浸透する特性がありますので注意してください。</li>";
  ...
  confModal +=
    '<tr class="pcOnly"><th>項　目</th><th class="btn_header">設 定 内 容</th></tr>';
  confModal +=
    '<tr><th>シリーズの未購入品を表示</th><td><div class="js_radioCookie js_notbuy radio_confModal_bookshelf"><span class="checked">す る</span><span>し な い</span></div></td></tr>';
  confModal +=
    '<tr><th>ＭＹ本棚の並び順</th><td><div class="js_radioCookie js_topItem_order radio_confModal_bookshelf"><span class="checked">タイトル名順</span><span>著者名順</span><span>購入日順</span></div></td></tr>';
  confModal +=
    '<tr><th>シリーズ内の並び順</th><td><div class="js_radioCookie js_detailItem_order radio_confModal_bookshelf"><span>新着順</span><span>巻数順</span><span class="checked">購入日順</span></div></td></tr>';
  confModal +=
    '<tr><th>表 示 件 数</th><td><div class="js_radioCookie js_display_num radio_confModal_bookshelf"><span class="checked">' +
    thisBookshelf.displyItemCount[0] +
    " 件</span><span>" +
    thisBookshelf.displyItemCount[1] +
    " 件</span><span>" +
    thisBookshelf.displyItemCount[2] +
    " 件</span></div></td></tr>";
  confModal +=
    '<tr><th>表 示 形 式</th><td><div class="js_radioCookie js_display_style radio_confModal_bookshelf"><span class="js_displayDetail">詳 細</span><span class="js_displayLarge checked">大</span></div></td></tr>';
  confModal += "</table>";
  confModal += "</div>";
  confModal += "</div>";
  return confModal;
...
Bookshelf.prototype.templateLabel = function(itemDataHash, bat) {
  if (thisBookshelf.device == "pc") {
    var wrapObject = $(".wrapper_bookshelf > div:eq(1)");
...
Bookshelf.prototype.template_itemList = function(
  itemDataHash,
  keyName,
  bat,
  no
  ...
  var modal = "";
  var btnType = "";
  var alterimgPath = thisBookshelf.alterImgSize(
    itemDataHash.thumbnail,
    "biggest"
  );
  var link;
  ...
  if (thisBookshelf.pageType == "top") {
    itemList = "<li>";
    link =
      "/test/pre_bookrenewal/html/bookshelf_detail_" +
      dev +
      ".html?id=" +
      thisBookshelf.userID +
      "&seriesId=" +
      keyName;
  } else if (thisBookshelf.pageType == "detail") {
    if (bat == false) {
  ...
  itemList +=
    '<img src="' + alterimgPath + '" alt="' + itemDataHash.item_name + '">';
  itemList += "</a>";
  ...
  if (thisBookshelf.pageType == "top") {
    ...
    itemList +=
      '<a href="javascript:void(0)">' + itemDataHash.publisher + "</a>";
    itemList +=
      '<a href="javascript:void(0)">' +
      itemDataHash.published_magazine +
      "</a>";
    if (
      itemDataHash.labal_name != undefined &&
      itemDataHash.labal_name != "undefined" &&
      itemDataHash.labal_name != ""
    ) {
      itemList +=
        '<a href="javascript:void(0)">' + itemDataHash.labal_name + "</a>";
    }
...
Bookshelf.prototype.parseUrl = function() {
  ...
  thisBookshelf.orderHistoryBeanDummyURL =
    "/test/pre_bookrenewal/js/dummy/shelf/orderHistoryBean" +
    thisBookshelf.userID +
    ".json";
...
Bookshelf.prototype.setDynamicParameter = function(param, value) {
  ...
  //総ページ数 totalPageNum
  thisBookshelf.totalPageNum = Math.ceil(
    thisBookshelf.totalItemNum / thisBookshelf.displyItemCount[Num]
  );
};
Bookshelf.prototype.pickBookHistory = function(data) {
  //callBack1 a-jax
  ...
  for (var i = 0; i < deliveryLists.length; i++) {
    ...
    for (var j = 0; j < eachDeliveryList.length; j++) {
      ...
      if (seriesId in thisBookshelf.preState) {
        thisBookshelf.preState[seriesId].push(
          eachDeliveryList[j].commodityCode
        );
      } else {
        var urlForSeries =
          "/test/pre_bookrenewal/js/dummy/shelf/" + seriesId + ".json";
        if (thisBookshelf.pageType == "top") {
          thisBookshelf.callAjax(
            urlForSeries,
            thisBookshelf.setSeriesState,
            seriesId
          );
        }
  ...
  if (thisBookshelf.pageType == "detail") {
    var urlForSeries =
      "/test/pre_bookrenewal/js/dummy/shelf/" +
      thisBookshelf.seriesId +
      ".json";
    thisBookshelf.callAjax(
      urlForSeries,
      thisBookshelf.setSeriesState,
...
Bookshelf.prototype.setSeriesState = function(data, seriesId) {
  //callBack2 a-jax
  ...
  if (thisBookshelf.pageType == "top") {
    ...
    for (var i = 0; i < books.length; i++) {
      ...
      if (i == 0) {
        ...
        thisBookshelf.preSeriesState[seriesId]["item_name"] =
          books[i].item_name;
        thisBookshelf.preSeriesState[seriesId]["thumbnail"] =
          books[i].thumbnail;
        thisBookshelf.preSeriesState[seriesId]["published_magazine"] = books[i]
          .published_magazine
          ? books[i].published_magazine
          : "";
        thisBookshelf.preSeriesState[seriesId]["labal_name"] = books[i]
          .published_magazine
          ? books[i].labal_name
          : "";
        thisBookshelf.preSeriesState[seriesId]["author"] = books[i].author;
        thisBookshelf.preSeriesState[seriesId]["title_name"] =
          books[i].title_name;
        thisBookshelf.preSeriesState[seriesId]["publisher"] =
          books[i].publisher;
        thisBookshelf.preSeriesState[seriesId]["latest_flg"] =
          books[i].latest_flg;
        ...
        thisBookshelf.preSeriesState[seriesId]["display_type"] =
          books[i].display_type;
      }
    ...
    thisBookshelf.setDynamicParameter(
      "totalItemNum",
      Object.keys(thisBookshelf.preSeriesState).length
    );
  } else if (thisBookshelf.pageType == "detail") {
    ...
    for (var i = 0; i < books.length; i++) {
      ...
      if (
        !isArray(thisBookshelf.seriesState[thisBookshelf.seriesId]["contents"])
      ) {
      ...
      thisBookshelf.seriesState[thisBookshelf.seriesId]["contents"].push(
        tempHash
      );
    }
  }
...
Bookshelf.prototype.rebuildContentsHash = function(type, name, value) {
  if (thisBookshelf.pageType == "top") {
    ...
    if (Object.assign) {
      thisBookshelf.cloneSeriesState = Object.assign(
        [],
        thisBookshelf.seriesState
      );
    } else {
      thisBookshelf.cloneSeriesState = $.extend([], thisBookshelf.seriesState);
    }
  } else {
    thisBookshelf.cloneSeriesState = {};
    if (Object.assign) {
...
Bookshelf.prototype.sortContentsHash = function(type, name, value) {
  if (
    name != "js_topItem_order" &&
    name != "js_detailItem_order" &&
    name != "js_notbuy"
  ) {
...
Bookshelf.prototype.parseDataToHtml = function(
  cloneSeriesState,
  itemStartNum,
  loopNum
) {
  var ulObj = $(".wrapper_bookshelf > div:eq(1) > ul");
  ...
  if (thisBookshelf.pageType == "top") {
    for (var i = 0; i < cloneSeriesState.length; i++) {
      ...
      if (itemStartNum <= i && i < endNum) {
        var batNum =
          Number(thisBookshelf.cloneSeriesState[i][keyName].total) -
          Number(thisBookshelf.preState[keyName].length);
        thisBookshelf.template_itemList(
          cloneSeriesState[i][keyName],
          keyName,
          batNum,
          ""
        );
    ...
    thisBookshelf.template_itemCount(
      itemStartNum,
      endNum,
      thisBookshelf.totalItemNum
    );
  }
  if (thisBookshelf.pageType == "detail") {
    var sid = thisBookshelf.seriesId;
...
Bookshelf.prototype.handleCookie = function(type, cookieName, activeSelect) {
  //cookie and globalVars
  if (type == "single") {
    ...
    var isdetailItem = thisBookshelf.cookieAction(
      "get",
      "js_detailItem_order",
      ""
    );
      ...
      if (cookieName == "loaded") {
        ...
        if (initialFlag == 0) {
          ...
          if (name == "js_now_page" || name == "js_display_num") {
            thisBookshelf.startNum =
              thisBookshelf.displyItemCount[
                thisBookshelf.itemParameter.js_display_num
              ] *
              (pageNum - 1);
          }
        } else {
...
Bookshelf.prototype.applyCookieToBtnDisplay = function(name, val, dispBoolean) {
  ...
  if (name == "js_now_page") {
    var val2 = val - 1;
    nowThis = $("." + name + "> span:eq(" + val2 + ")");
    //  console.log('1: '+name);
  } else {
    nowThis = $("." + name + "> span:eq(" + val + ")");
  }
  ...
  if (name == "js_display_style") {
    ...
    if (val == 1) {
      ...
      $(".js_displayLarge").addClass("checked");
      $(".js_displayDetail").removeClass("checked");
      ...
      $(".js_displayLarge").removeClass("checked");
      $(".js_displayDetail").addClass("checked");
      //labelとcartの縦横CSS切り替え
      if (thisBookshelf.device == "pc") {
        var label = $(".status");
        var label_vertical = label.hasClass("vertical");
...
Bookshelf.prototype.calcAndDisplayContents = function(name) {
  ...
  var displayNum =
    thisBookshelf.displyItemCount[thisBookshelf.itemParameter.js_display_num];
...
Bookshelf.prototype.setParamAndCookiebtnAction = function(
  type,
  clickedBtnObjectOrLoadName,
  loadValue
) {
  var thisBtnObject;
  var val;
  ...
  if (nowTarget.indexOf("js_display_num") != -1) {
    ...
    if (type == "click") {
      thisBookshelf.rebuildContentsHash("multi", "loaded", {
        js_display_num: 0,
        js_now_page: 0,
      });
    }
    ...
    if (nowTarget.indexOf("js_now_page") != -1) {
      ...
      thisBookshelf.setDynamicParameter("js_now_page", nextNowThisPage);
      thisBookshelf.cookieAction("set", "js_now_page", nextNowThisPage);
      thisBookshelf.rebuildContentsHash(
        "single",
        "js_now_page",
        nextNowThisPage
      );
    } else if (
      nowTarget.indexOf("js_notbuy") != -1 ||
      nowTarget.indexOf("js_display_style") != -1 ||
      nowTarget.indexOf("js_topItem_order") != -1 ||
      nowTarget.indexOf("js_detailItem_order") != -1 ||
      nowTarget.indexOf("js_notbuy") != -1
    ) {
...
Bookshelf.prototype.btnAction = function() {
  ...
  $(document).ready(function() {
    $(document).on("click", ".js_radioCookie > span", function() {
      thisBookshelf.setParamAndCookiebtnAction("click", $(this));
    });
  });
  //cart modal display
  $(document).on("click", ".js_modalCartOpen, .js_modalLinkOpen", function() {
    var thisThumb = $(this);
    var targetModalName;
    ...
    $(".bookThum > li").each(function(index, element) {
      var otherModalNum = returnParentNum($(element));
      if (otherModalNum != modalNum) {
        $(".item" + otherModalNum)
          .find(".modalBox_bookshelf")
          .hide();
      } else {
        var thisModal = $(".item" + otherModalNum).find(
          "." + targetModalName + "_bookshelf"
        );
  ...
  //cart modal display
  $(document).on("click", ".js_openBookViewer", function() {
    location.href = "./browseViewer.html";
  });
  //conf modal display
  $(document).on("click", ".js_conf", function() {
    var js_confModal_bookshelf = $(".js_confModal_bookshelf");
  ...
  $(document).on("click", ".js_confModal_close", function() {
    $(this)
      .parent()
      .css({
        display: "none",
      });
    $(".js_conf").removeClass("checked");
  });
  //filter
  $(document).on("focus", "#filter_bookshelf", function(e) {
    if (e.type == "focusin") {
      thisBookshelf.filterElement = "";
      thisBookshelf.filterMyBook();
    }
  });
  $(document).on("click", ".js_filtedItem_close", function(e) {
    $(".js_filtedItemBox").hide();
    $(".js_filtedItemBox > ul").empty();
    thisBookshelf.filterElement = "";
  });
};
Bookshelf.prototype.filterMyBook = function() {
  $(document).on("change", "#filter_bookshelf", function() {
    var word = $(this).val();
    if (thisBookshelf.pageType == "top") {
      var filtered = $.grep(thisBookshelf.myBookState, function(hash, index) {
        if (hash["item_name"].indexOf(word) != -1) {
          attachItem(hash);
        }
      });
    }
    $(".js_filtedItemBox").show();
    $(".js_filtedItemBox > ul").append(thisBookshelf.filterElement);
    $(".js_filtedItemBox").css({ background: "rgba(23, 83, 10, 0.1)" });
    thisBookshelf.filterElement = "";
  });
  function attachItem(itemHash) {
    thisBookshelf.filterElement += "<li>";
    thisBookshelf.filterElement +=
      '<a href="./browserViewer.html?' + itemHash.item + '">';
    thisBookshelf.filterElement +=
      '<img src="' +
      itemHash.thumbnail +
      '" data-src="' +
      ...
      '">';
    thisBookshelf.filterElement +=
      '<p class="ttl_filter_bookshelf">' + itemHash.item_name + "</p>";
...
Bookshelf.prototype.loadAction = function() {
  ...
  var display_listLarge_book =
    '<div class="display_listLarge_book"><ul class="bookThum"></ul></div>';
  var listNav = $('<div class="listNav"></div>');
  listNav.append(ttlBox).append(listNav_inner2);
  $("main").prepend(listNav);
  $(".wrapper_bookshelf").append(display_listLarge_book);
  $("body").append(confModal);
  if (thisBookshelf.pageType == "detail") {
    var ttl = thisBookshelf.pageTitle + "　全 " + thisBookshelf.total + " 巻";
    $(".js_seriesName").text(ttl);
  }
  //■LOAD　すべて
