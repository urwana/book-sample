...
Bookseries.prototype.separateElementOut = function (type) {
    ...
    var labelStatus = j$('.status');
    var addCartMsg = j$('.addCartMsg');
    ...
    if (thisBookseries.pageType == 'pc_detail') {
        if (type == 'js_displayLarge') {
            j$('.authorSet ').css({ 'display': 'block' });
            j$('.authorSet > a').css({ 'display': 'none' });
            j$('.authorSet > a:nth-of-type(1)').css({ 'display': 'block' });
            j$('.bookThumComment').css({ 'display': 'none' });
            j$('.priceGroup_book').css({ 'display': 'none' });
        } else {
            j$('.authorSet').css({ 'display': 'none' });
            j$('.bookThumComment').css({ 'display': 'block' });
            j$('.priceGroup_book').css({ 'display': 'block' });
        }
    } else if (thisBookseries.pageType == 'sp_detail') {
        j$('.authorSet ').css({ 'display': 'none' });
        if (type == 'js_displayLarge') {
            //j$('.bookThumTool').css({ 'display': 'block' });
            j$('.priceGroup_book').css({ 'display': 'none' });
            //j$('.authorSet').css({ 'display': 'none' });
            j$('.bookThumData .name').css({ 'display': 'none' });
        } else {
            //j$('.bookThumTool').css({ 'display': 'block' });
            j$('.priceGroup_book').css({ 'display': 'block' });
            //j$('.authorSet').css({ 'display': 'block' });
            j$('.bookThumData .name').css({ 'display': 'block' });
        }
    } else if (thisBookseries.pageType == 'pc_ranking' || thisBookseries.pageType == 'pc_search') {
        if (type == 'js_displayLarge') {
            j$('.bookThumComment').css({ 'display': 'none' });

            j$('.bookCartListBtnNew').css({ 'display': 'none' });
            j$('.bookCartListBtn').css({ 'display': 'none' });
            j$('.bookReserveListBtn').css({ 'display': 'none' });
            j$('.bookReserveListBtnNew').css({ 'display': 'none' });

            j$('.priceGroup_book').css({ 'display': 'none' });
            j$('.linkToSeries').css({ 'display': 'none' });
            j$('.authorSet .genre').css({ 'display': 'none' });
            j$('.authorSet > a').css({ 'display': 'none' });
            j$('.authorSet > a:nth-of-type(1)').css({ 'display': 'block' });
            j$('.authorSet > a:nth-of-type(2)').css({ 'display': 'none' });
        } else {
            j$('.bookThumComment').css({ 'display': 'block' });

            j$('.bookCartListBtnNew').css({ 'display': 'inline-block' });
            j$('.bookCartListBtn').css({ 'display': 'inline-block' });
            j$('.bookReserveListBtn').css({ 'display': 'inline-block' });
            j$('.bookReserveListBtnNew').css({ 'display': 'inline-block' });

            j$('.priceGroup_book').css({ 'display': 'block' });
            j$('.linkToSeries').css({ 'display': 'block' });
            j$('.authorSet .genre').css({ 'display': 'block' });
            j$('.authorSet > a').css({ 'display': 'none' });
            j$('.authorSet > a:nth-of-type(1)').css({ 'display': 'block' });
            j$('.authorSet > a:nth-of-type(2)').css({ 'display': 'table' });
        }
    } else if (thisBookseries.pageType == 'sp_ranking' || thisBookseries.pageType == 'sp_search') {
        if (type == 'js_displayLarge') {
            j$('.bookCartListBtnNew').css({ 'display': 'none' });
            j$('.bookCartListBtn').css({ 'display': 'none' });
            j$('.bookReserveListBtn').css({ 'display': 'none' });
...
Bookseries.prototype.parseDataToHtml = function (itemDataHash) {
    if (thisBookseries.pageType == 'pc_detail' || thisBookseries.pageType == 'sp_detail') {
    ...
    } else if (thisBookseries.pageType == 'pc_search' || thisBookseries.pageType == 'sp_search') {
        var contents = itemDataHash.contents;
        for (var i = 0; i < contents.length; i++) {
            thisBookseries.templateItemList(contents[i]);
        }
    } else if (thisBookseries.pageType == 'pc_ranking' || thisBookseries.pageType == 'sp_ranking') {
...
Bookseries.prototype.templateDetail = function (itemDataHash) {
    ...
    j$('.js_lead_detail_book').text(leadTxt);
};
    ...
    */
    if (itemDataHash.display_type == "single") {
        ...
        if (itemDataHash.icon_list[4] == 1) {
            thisBookseries.tempLabel += '<span class="cat-discount"><span>20%</span><span>OFF</span></span>';
        }
        ...
        if (itemDataHash.icon_list[7] == 1) {
            thisBookseries.tempLabel += '<span class="cat-free">無料＆割引あり</span>';
        }
        if (itemDataHash.icon_list[8] == 1) {
            thisBookseries.tempLabel += '<span class="cat-end">完結</span>';
        }
...
Bookseries.prototype.templateItemList = function (itemDataHash, display_type, seriesName) {
    ...
    if (itemDataHash.icon_list[1] == 1) {
        itemList += '<a href="/test/pre_bookrenewal/html/browseViewer.html?' + itemDataHash.item + '">';
    } else if (itemDataHash.icon_list[1] == 2) {
        itemList += '<a href="/test/pre_bookrenewal/html/appViewer.html?' + itemDataHash.item + '">';
    } else {
        itemList += '<a href="' + itemDataHash.detail_url + '"class="photo">';
    }
    itemList += '<img src="' + thisBookseries.alterImgSize(itemDataHash.thumbnail, 'biggest') + '" alt="' + itemDataHash.item_name + '">';
    itemList += '</a>';
    ...
    */
    ...
    if (thisBookseries.pageType == 'pc_ranking' || thisBookseries.pageType == 'sp_ranking') {
        if (1 <= itemDataHash.rank && itemDataHash.rank <= 3) {
            itemList += '<div class="rank"><img src="/test/pre_bookrenewal/img/common/icon_rank0' + itemDataHash.rank + '.gif"><span>' + itemDataHash.rank + '位</span></div>';
        } else {
            //itemList += '<div class="rank"><img src="/test/pre_bookrenewal/img/icon_ranki00.gif"><span>' + itemDataHash.rank + '位</span></div>';
            itemList += '<div class="rank"><span>' + itemDataHash.rank + '位</span></div>';
        };
    };
    ...
    if (itemDataHash.display_type == "series") {
        itemList += '<a href="javascript:searchEbookCommodity(\'\'\,\'' + itemDataHash.title_name + '\'\,\'\')" class="name">';
        itemList += '<span>' + itemDataHash.title_name + '</span>';
        itemList += '</a>';
    } else {
        if (itemDataHash.icon_list[1] == 1) {
            itemList += '<a href="/test/pre_bookrenewal/html/browseViewer.html?' + itemDataHash.item + '" class="name">';
        } else if (itemDataHash.icon_list[1] == 2) {
            itemList += '<a href="/test/pre_bookrenewal/html/appViewer.html?' + itemDataHash.item + '" class="name">';
        } else {
            itemList += '<a href="https://testbook.hikaritv.net/book/content/book-store/9000109354/" class="name">';
        }
    ...
    if (thisBookseries.pageType != 'sp_detail') {
        ...
        itemList += '<a href="javascript:searchEbookCommodity(' + autherForFunc + ') ">' + itemDataHash.author + '</a>';
        if (thisBookseries.pageType == 'pc_ranking' || thisBookseries.pageType == 'pc_search') {
            itemList += '<a href="javascript:searchEbookCommodity(' + publisher + ')">' + itemDataHash.publisher + '</a>';
            itemList += '<span class="genre">ビジネス・実用等 &gt;';
            itemList += '<a href="https://book.hikaritv.net/book/app/catalog/search/init?searchCategoryCode=bk142" onmousedown="this.href += \' & amp; pageSize = 15\'">' + itemDataHash.genre_name + '</a>';
            itemList += '</span>';
        }
        itemList += '</span>';
    }
    if (thisBookseries.pageType == 'pc_search' || thisBookseries.pageType == 'sp_search' || thisBookseries.pageType == 'pc_ranking' || thisBookseries.pageType == 'sp_ranking') {
        itemList += '<div class="linkToSeries">';
        itemList += '<a href="/test/pre_bookrenewal/pc_search_series.html?' + itemDataHash.series_id + '">「' + itemDataHash.series_name + '」シリーズへ</a>';
        itemList += '</div>';
    }
    if (thisBookseries.pageType == 'pc_detail' || thisBookseries.pageType == 'sp_detail') {
        if (itemDataHash.now_price < itemDataHash.unit_price) {
            ...
            itemList += '<tbody>';
            itemList += '<tr><th>価格</th><td>：</td><td class="price breakThrough"><span>' + itemDataHash.unit_price + '</span><span>円</span><span class="unit">（税込）</span></td></tr>';
            itemList += '<tr>';
            itemList += '<th>キャンペーン価格</th><td>：</td><td class="price highLight"><span>' + itemDataHash.now_price + '</span><span>円</span><span class="unit">（税込）</span></td>';
            itemList += '</tr>';
            itemList += '</tbody>';
            itemList += '</table>';
            itemList += '</div>';
            ...
            itemList += '<table class="table_priceAndPoint">';
            itemList += '<tbody>';
            itemList += '<tr><th>価格</th><td>：</td><td class="price"><span>' + itemDataHash.unit_price + '</span><span>円</span><span class="unit">（税込）</span></td></tr>';
            itemList += '</tbody>';
            itemList += '</table>';
            itemList += '</div>';
        }
        ...
        if (itemDataHash.display_type == "single") {
            ...
            itemList += '<table class="table_priceAndPoint">';
            itemList += '<tbody>';
            itemList += '<tr><td class="highLight2"><span>' + itemDataHash.unit_price + '</span><span>円</span><span class="unit">（税込）</span></td></tr>';
            itemList += '</tbody>';
            itemList += '</table>';
            itemList += '</div>';
        } else if (itemDataHash.display_type == "series") {
            ...
            itemList += '<table class="table_priceAndPoint">';
            itemList += '<tbody>';
            itemList += '<tr><td class="highLight2"><span>' + itemDataHash.now_price + ' ～ ' + itemDataHash.unit_price + '</span><span>円</span><span class="unit">（税込）</span></td></tr>';
            itemList += '</tbody>';
            itemList += '</table>';
            itemList += '</div>';
        }
    ...
    if (itemDataHash.point_magnification > 1 && (itemDataHash.sales_date == '' || itemDataHash.sales_date == undefined)) {
        if (itemDataHash.display_type == "series") {
            itemList += '<div class="btn point">ポイント' + itemDataHash.point_magnification + '倍あり</div>';
        } else if (itemDataHash.display_type == "single") {
            itemList += '<div class="btn point">ポイント' + itemDataHash.point_magnification + '倍</div>';
        }
    } else {
        if (itemDataHash.sales_date) {
            itemList += '<span class="releaseDate">' + itemDataHash.sales_date + ' 発売予定</span>';
        } else {
            itemList += '<span class="releaseDate" style="visibility:hidden;"></span>';
        };
    }
    if (reviewFlag) {
        itemList += '<dl class="reviewRate">';
        itemList += '<dt class="rateStatus rateStatus0"></dt>';
        itemList += '<dd class="rateNum"><a href="http://lab-www.hikaritv.net/search/book/request/00/review/Ym9vay85MDAwMTA5MzU0/">0</a></dd>';
        itemList += '<dd class="rateTotal"><span>(0)</span></dd>';
        itemList += '<dd class="reviewNum"><a href="http://lab-www.hikaritv.net/search/book/request/00/review/Ym9vay85MDAwMTA5MzU0/">0件</a></dd>';
        itemList += '</dl>';
    }
    itemList += '</span>';//bookThumData END
    if (thisBookseries.device == "pc") {
        itemList += '<span class="bookThumComment"><h2>作品内容</h2>' + itemDataHash.detail + '</span>';
    }
    //bookThumTool
    ...
    if (itemDataHash.display_type == "single") {
        if (itemDataHash.icon_list[1] == 0) {
            if (itemDataHash.sales_date) {
                ...
                itemList += '</a>';
                itemList += '<span class="addMsg">book-store_' + itemDataHash.item + '</span>';
            ...
            itemList += '</a>';
            itemList += '<span class="addMsg">book-store_' + itemDataHash.item + '_' + pageNum + '_' + pageType + '</span>';
        ...
        if (!itemDataHash.sales_date && thisBookseries.device == 'pc') {
            if (itemDataHash.icon_list[9] == 0) {
                ...
                itemList += '<a href="https://book.hikaritv.net/browser_viewer/sample/index.html#cid=BT000012354205005001900206&amp;pid=pbrgargt&amp;title=%EF%BC%A7%EF%BC%A9%EF%BC%A1%EF%BC%AE%EF%BC%B4%E3%80%80%EF%BC%AB%EF%BC%A9%EF%BC%AC%EF%BC%AC%EF%BC%A9%EF%BC%AE%EF%BC%A7%EF%BC%88%EF%BC%95%EF%BC%90%EF%BC%89&amp;url=https%3A%2F%2Fbook.hikaritv.net%2Fbook%2Fcontent%2Fbook-store%2F9000720290">';
                itemList += '<span>【無料サンプル】<br class="pc_none">最新刊を<br>ブラウザ試し読み(bw)</span>';
                itemList += '</a>';
                itemList += '</li>';
            } else if (itemDataHash.icon_list[9] == 1) {
                itemList += '<li class="bookReadTrialBtnBrowserNew">';
                itemList += '<a href="https://book.hikaritv.net/browser_viewer/sample/index.html#cid=BT000012354205005001900206&amp;pid=pbrgargt&amp;title=%EF%BC%A7%EF%BC%A9%EF%BC%A1%EF%BC%AE%EF%BC%B4%E3%80%80%EF%BC%AB%EF%BC%A9%EF%BC%AC%EF%BC%AC%EF%BC%A9%EF%BC%AE%EF%BC%A7%EF%BC%88%EF%BC%95%EF%BC%90%EF%BC%89&amp;url=https%3A%2F%2Fbook.hikaritv.net%2Fbook%2Fcontent%2Fbook-store%2F9000720290">';
                itemList += '<span>【無料サンプル】<br class="pc_none">最新刊を<br>ブラウザ試し読み(ap)</span>';
                itemList += '</a>';
                itemList += '</li>';
            }
        }
...
Bookseries.prototype.btnAction = function () {
    j$('.dummyLog').on('click', function (e) {
        //dummy
        if (thisBookseries.login == 0) {
            j$('.bookAllBuyBtn .exArea > a:nth-of-type(1)').hide();
            j$('.bookAllBuyBtn .exArea > a:nth-of-type(2)').show();
            thisBookseries.login = 1;
        } else if (thisBookseries.login == 1) {
            j$('.bookAllBuyBtn .exArea > a:nth-of-type(2)').hide();
            j$('.bookAllBuyBtn .exArea > a:nth-of-type(1)').show();
            thisBookseries.login = 0;
        }
    });
    j$(document).on('click', '.js_readNext_book', function () {
        thisBookseries.readNextButton('click', j$(this));

    });
    j$(document).on('click', '.list_ranking_controller_sub >li, .list_ranking_controller_main > li', function () {
    ...
    j$(document).on('click', '.js_display_style >span', function () {
        var wrapObject = j$('.listBookArticle > div');
        var thisButton = j$(this);
        thisButton.siblings().removeClass('checked');
        if (!thisButton.hasClass('checked')) {
            thisButton.addClass('checked');
    ...
    j$(document).on('click', '.js_menu_opener', function () {
        var thisCheckbox = j$(this).find('.js_subMenuOpener');
        var nowClass = j$(this).attr('class');
        var sib;
        if (nowClass.indexOf('2') != -1) {
            sib = j$(this).siblings('ul');
        } else {
            sib = j$(this).siblings('div');
        }
    ...
    j$(document).on('click', '.js_filterMoreButton', function () {
        j$('#list').submit();
    });
    j$(document).on('click', '.js_filterResetButton', function () {
        j$('.nav_filter_book input').prop('checked', false);
    });
    if (thisBookseries.device == 'pc') {
        j$(document).on('click', '.hierarchy_1 input, .hierarchy_2 input', function () {
            thisBookseries.inputStyle('click', j$(this));
        });
    } else {
        j$(document).on('click', '.js_filterButton', function () {
            var filterBox = j$('.js_nav_filterBox_book');
            var filOpen = filterBox.css('display');
            if (filOpen == 'none') {
                filterBox.show();
                j$('.js_filter_open_book').hide();
                j$('.js_filter_close_book').show();
            } else {
                filterBox.hide();
                j$('.js_filter_open_book').show();
                j$('.js_filter_close_book').hide();
            }
        });
    }
    j$(document).on('click', '.js_searchNavSwitcher > li', function () {
        if (j$(this).hasClass('year')) {
            j$('.js_rankController_sub').css({
                'display': 'none'
            });
            j$('.js_rankController_year').css({
                'display': 'block'
            });
        } else {
            j$('.js_rankController_sub').css({
...
Bookseries.prototype.readNextButton = function (actionType, targetObject) {
    ...
    if (actionType == 'load') {
        if (thisBookseries.pageType == 'sp_detail') {
            ...
            thisBookseries.itemExplain_txt100_book = thisBookseries.adjustText(thisBookseries.itemExplain_txt_book, 100);
...
Bookseries.prototype.inputStyle = function (status, inputObject) {
    if (status == 'load') {
        j$('.hierarchy_1 input, .hierarchy_2 input').css({
            "appearance": "none",
...
Bookseries.prototype.loadAction = function () {
    ...
    if (thisBookseries.pageType == 'sp_search') {
        j$('.js_nav_filterBox_book').css({
            'display': 'none'
        });
    };
    if (thisBookseries.pageType == 'pc_detail') {
        thisBookseries.readNextButton('load', j$('.js_readNext_book'));
    } else if (thisBookseries.pageType == 'sp_detail') {
        thisBookseries.readNextButton('load', j$('.js_itemExplain_txt_book'));
    }
    thisBookseries.series_id = j$('input[name=series_id]').val();
    if (thisBookseries.pageType == 'pc_detail' || thisBookseries.pageType == 'sp_detail') {
