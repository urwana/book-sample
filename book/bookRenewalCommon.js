...
BookRenewalCommon.prototype = {
    uaJudge: function () {
        if (thisBookRenewalCommon.ua.indexOf('iphone') !== -1 || thisBookRenewalCommon.ua.indexOf('ipod') !== -1 || thisBookRenewalCommon.ua.indexOf('ipad') !== -1) {
        ...
        } else if (thisBookRenewalCommon.ua.indexOf('windows') !== -1 && thisBookRenewalCommon.ua.indexOf('phone') !== -1) {
    ...
    getWindowParam: function () {
        thisBookRenewalCommon.displayWidth = j$(window).innerWidth();
        thisBookRenewalCommon.displayHeight = j$(window).innerHeight();
        thisBookRenewalCommon.Bottom = thisBookRenewalCommon.displayHeight;
    },
    leaderThree: function () {
        j$('.leader3').each(function () {
            var bookRenewalCommonTarget = j$(this);
            ...
            if (bookRenewalCommonTtlClone.height() > bookRenewalCommonTarget.height()) {
                while ((html.length > 0) && (bookRenewalCommonTtlClone.height() > bookRenewalCommonTarget.height())) {
    ...
    lazyLoader: function (itemObject, nextPosition, eventType) {
        ...
        function lazyAction(thisImgObj, doneLz) {
            if (doneLz) {
                ...
                if (0 <= thisTop && thisTop <= thisBookRenewalCommon.displayHeight && 0 <= thisLeft && thisLeft <= thisBookRenewalCommon.displayWidth) {
                    if (thisSrc) {
    ...
    parseUrl: function () {
        ...
        thisBookRenewalCommon.orderHistoryBeanDummyURL = "/test/pre_bookrenewal/js/dummy/orderHistoryBean" + thisBookRenewalCommon.userID + ".json";
    ...
    cookieAction: function (type, cookieName, value) {
        ...
        if (type == 'set') {
            ...
            var dtClTime = new Date(dtExTime + (60 * 60 * 24 * 1000 * thisBookRenewalCommon.cookieExpire));
