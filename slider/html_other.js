...
PlalaSliderHtml.prototype.perseDataToHtml = function (itemDatas, target, dataType, doLazy, lazyItemCount) {
    ...
    itemDatas.forEach(function (itemData, index) {
        ...
        //■ ポイントの計算と表示
        sliderElement += '<div class="pointGroup">';
        var pointGet = Math.floor(itemData.now_price / 100) * itemData.point_magnification;
        if (itemData.point_magnification >= 2) {
            sliderElement += '<div class="point_plalaSlider">ポイント' + itemData.point_magnification + '倍</div>';
        }
        if (itemData.now_price && itemData.point_magnification) {
            var getPT = '<div class="getPT">' + pointGet + '<span class="getPT_s"> PTゲット!</span></div>';
            sliderElement += getPT;
        }
        sliderElement += '</div>';
        //■ item_txtBox
        ...
        sliderElement += '<span class="txt_itemPrice_plalaSlider">&yen; ' + thisPlalaSliderHtml.insertCommaInNum(itemData.unit_price) + ' &rarr;</span>';
        sliderElement += '<span class="txt_itemPriceOff_plalaSlider"><span class="item_priceOffEn_plalaSlider">￥</span>' + thisPlalaSliderHtml.insertCommaInNum(itemData.now_price) + '</span>';
        sliderElement += '</p>';
        sliderElement += '</div>';
